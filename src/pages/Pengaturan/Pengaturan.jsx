              ...
              <div className="flex flex-row lg:flex-col gap-1 overflow-x-auto pb-2 lg:pb-0">
                {["profil", "keamanan", "notifikasi"].map((tab) => (
                  <button
                    ...
                    className={`whitespace-nowrap lg:text-left px-4 py-2.5 rounded-lg text-sm transition-all font-medium border-b-4 lg:border-b-0 lg:border-l-4 capitalize ${
                      activeTab === tab ? "bg-blue-50/50 text-blue-700 border-blue-600" : "border-transparent text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                    }`}
                  >
                  ...
                  <div className="px-8 pb-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                      <div>
                        ...
                        <input type="text" name="name" value={profile.name} onChange={handleInputChange} className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-500" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Email Akses</label>
                        <input type="email" name="email" value={profile.email} onChange={handleInputChange} className="w-full px-4 py-2.5 rounded-lg border border-gray-300 bg-gray-50 text-gray-500 text-sm cursor-not-allowed" disabled />
                      </div>
                    </div>

                    <div className="mt-4">
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">Bio Singkat</label>
                      <textarea name="bio" value={profile.bio} onChange={handleInputChange} rows="3" className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-500 resize-none" placeholder="Ceritakan sedikit tentang dirimu..." />
                    </div>
                  </div>
                </div>
              )}
